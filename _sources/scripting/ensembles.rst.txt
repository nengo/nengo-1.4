...
When creating a neural ensemble,
...
of the transformations computed by the neurons.

Membrane time constant and refractory period
--------------------------------------------

The two parameters for leaky integrate-and-fire neurons
are the membrane time constant (``tau_rc``)
and the refractory period (``tau_ref``).
...
Empirical data on the membrane time constants
for different types of neurons
in different parts of the brain
can be found `here
<http://compneuro.uwaterloo.ca/research/constants-constraints/membrane-time-constant-tau_rc.html>`_.

Maximum firing rate
-------------------

You can also specify the maximum firing rate for the neurons.
...
is the *normal* operating range for the neurons.
More technically, this is the maximum firing rate
*assuming that the neurons are representing
...
with maximums between 200 Hz and 400 Hz::
...
the provided values will be re-used::

    net.make('F', 100, 2, max_rate=[200, 250, 300, 350, 400])

.. note:: The type of brackets used is important!
          ...
          and square brackets create a *list*,
          which we use to specify a list of particular values to use.

Intercept
---------

The intercept is the point on the tuning curve graph
...
if the dot product of **x** (the value being represented)
...
the intercepts will always be normalized to be between -1 and 1.

While this parameter can be used
...
and other functions where many of the output values are zero.

By default, intercepts are uniformly distributed between -1 and 1.
...
.. note:: As with the maximum firing rate,
          the type of brackets used is important!
          ``()`` indicates a range of values,
          while ``[]`` indicates a list of specific values to use.

Encoders (preferred direction vectors)
--------------------------------------

You can specify the encoders
...
The encoders given will automatically be normalized to unit length::

  net.make('F', 100, 2, encoders=[[1, 0], [-1, 0], [0, 1], [0, -1]])
  net.make('G', 100, 2, encoders=[[1, 1], [1, -1], [-1, 1], [-1, -1]])

This allows you to make complex sets of encoders
...
and the other half of which are aligned on the diagonals::
    ...
    encoders = []
    for i in range(50):
        # choose a random direction between -pi and pi
        theta = random.uniform(-math.pi, pi)
        # add the encoder
        encoders.append([math.sin(theta), math.cos(theta)])

    for i in range(50):
        # add an aligned encoder
        encoders.append(random.choice([[1, 1], [1, -1], [-1, 1], [-1, -1]]))

    net.make('G', 100, 2, encoders=encoders)
