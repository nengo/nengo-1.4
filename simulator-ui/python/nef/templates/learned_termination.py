...
description = ("<html>This template introduces error-modulated learning "
               "into a pre-existing connection in the network. The "
               "presynaptic side is 'pre' and the postsynaptic side is "
               "'post'. The error signal must be computed in the added "
               "error ensemble, and is usually a gradient.</html>")

params = {
    'errName': PString(
        'Name of (new) error ensemble',
    ...
    'N_err': PInt(
        'Number of neurons in error ensemble',
        'Set the number of neurons in the error ensemble',
        50,
        1,
        sys.maxint,
    ),
    'preName': PString(
        'Name of pre ensemble',
        ('Select the name of the pre ensemble '
         '(this ensemble must already exist)'),
        'pre',
    ),
    'postName': PString(
        'Name of post ensemble',
        ('Select the name of the post ensemble '
         '(this ensemble must already exist)'),
        'post',
    ),
    'rate': PFloat(
...
def test_params(net, p):
    ...
    for i in nodeList:
        if i.name == p['preName']:
            ...
            nameIsTaken = True
    if nameIsTaken:
        return 'The name for error ensemble is already taken'
    if not preIsSet:
        return 'Must provide the name of an existing pre ensemble'
    if not postIsSet:
        return 'Must provide the name of an existing post ensemble'
    if p['N_err'] < 1:
        return 'Number of dimensions must be at least one'
