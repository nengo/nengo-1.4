...
description = ("<html>This template is dropped onto a basal ganglia model "
               "generated from the Basal Ganglia template. It is used to "
               "define input rules in an SPA model. It provides the mapping "
               "from cortex to striatum to determine the value of the input "
               "semantic pointer with respect to the indexed rule.</html>")

params = {
    'index': PInt(
        'Rule Index',
        'The number of the rule currently being defined (0 indexed)',
        0,
    ),
    'pattern': PString(
        'Semantic Pointer',
        'The input representation that activates the rule',
        'I',
    ),
    'dimensions': PInt(
        'Dimensionality',
    ...
    'pstc': PFloat(
        'Input PSTC [s]',
        'Post-synaptic time constant for input filtering, in seconds',
        0.01,
        1e-40,
        float("inf"),
    ),
    'use_single_input': PBoolean(
        'Use Single Input',
        'Use a converged input rather than seperate inputs to striatum and STN',
        False,
    ),
}

def test_params(net, node, p):
    if p['index'] > node.getNode('STN').dimension:
        return 'Rule index cannot exceed basal ganglia dimension minus one (use a zero-based index)'
