...
class TemplateConstructor(IConfigurable):
    ...
    def set_network(self, network, node):
        if network is None:
            ...
            self.network = nef.Network(network)
        self.node = node
...
class DropHandler(TransferHandler, java.awt.dnd.DropTargetListener):
    ...
    def canImport(self, support, flavors=None):
        ...
        if constructor is ca.nengo.ui.configurable.models.CNetwork:
            return True

        drop_on_ensemble = False
        if constructor is ca.nengo.ui.configurable.models.CDecodedOrigin:
            drop_on_ensemble = True
        if constructor is ca.nengo.ui.configurable.models.CDecodedTermination:
            drop_on_ensemble = True
