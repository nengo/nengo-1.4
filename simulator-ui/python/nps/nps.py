...
class NPS:
    def __init__(self,net,productions,dimensions,neurons_buffer=40,neurons_bg=40,neurons_product=300,tau_gaba=0.008,tau_ampa=0.002,noise=None):
        ...
        """    
        for a,b in productions.rhs_routes:
            name='thal_%s_%s'%(a,b)
            net.make(name,neurons_buffer*dimensions,dimensions,quick=True)
            net.connect(prod,name,transform=calc_output_transform(productions,'%s_to_%s'%(a,b),vocab),pstc=tau_ampa)
            conv=convolution.make_convolution(net,'%s_to_%s'%(a,b),name,'buffer_'+a,'buffer_'+b,1,quick=True)
            
            from ca.nengo.model import SimulationMode
            conv.mode=SimulationMode.DIRECT
        """    
    ...
    def add_buffer_feedback(self,**args):
        for k,v in args.items():
            self.net.connect('buffer_'+k,'buffer_'+k,weight=v)
