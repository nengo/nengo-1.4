...
input=FunctionInput('input',[FourierFunction(.1, 8,.4,i, 0) for i in range(D)],Units.UNK) #Create a white noise input function .1 base freq, max freq 8 rad/s, and RMS of .4; i makes one for each dimension; 0 is the seed
...
net.make_input('switch',[0]) #Create a controllable input function with a starting value of 0 and 0 in the two dimensions
gating.make(net,name='Gate', gated='error', neurons=40 ,pstc=0.01) #Make a gate population with 40 neurons, and a postsynaptic time constant of 10ms
