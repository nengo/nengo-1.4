...
public class FourierFunction implements Function {
	...
	/**
	 * Creates a band-limited pink noise function with specified parameters. 
	 *  
	...
	public FourierFunction(float fundamental, float cutoff, float rms) {
		...
		myFrequencies = new float[n];
		myAmplitudes = new float[n];
		myPhases = new float[n];
		
		for (int i = 0; i < n; i++) {
			myFrequencies[i] = fundamental * (i+1);
			myAmplitudes[i] = (float) Math.random() * fundamental / myFrequencies[i]; //decreasing amplitude = pink noise
//			myPhases[i] = (float) ( -Math.PI + (2d *  Math.PI * Math.random()) ) ;
			myPhases[i] = -.5f + 2f * (float) Math.random();
		}
		...
		for (int i = 0; i < samplePoints; i++) {
			float val = getValue(i*dx, myFrequencies, myAmplitudes, myPhases);
			sumSquared += val * val;
		}
	...
	public float map(float[] from) {
		return getValue(from[0], myFrequencies, myAmplitudes, myPhases);
	}
	...
	public float[] multiMap(float[][] from) {
		...
		for (int i = 0; i < from.length; i++) {
			result[i] = getValue(from[i][0], myFrequencies, myAmplitudes, myPhases);
		}
	...
	private static float getValue(float x, float[] f, float[] a, float[] p) {
		...
		for (int i = 0; i < f.length; i++) {
			result += a[i] * (float) Math.sin(2d * Math.PI * (f[i] * x + p[i]));
		}
