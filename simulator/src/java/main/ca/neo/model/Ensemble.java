 ...
 * 
 * <p>There are no strict rules for how to group Nodes into Ensembles, 
 * but here are some things to consider: 
 * 
 * <ul><li>A group of Nodes that together 'represent' something through 
 ...
 * 
 * <li>Outputs from an Ensemble are grouped together and passed to 
 * other Ensembles during a simulation, and practical issues may arise 
 * from this. For example, putting all your Nodes in a single large ensemble 
 * could result in a very large matrix of synaptic weights, which would impair 
 * performance. </li></ul>  
 * </p>
 * 
 * <p>The membership of an Ensemble is fixed once the Ensemble is created. This means 
 * that the Ensemble model doesn't deal explicitly with growth and death of components
 * during simulation (although you can set input/output weights to zero to mimic this).
 * It also means that an Ensemble isn't a good model of a functional "assembly".</p>
 * 
 * @author Bryan Tripp
 */
public interface Ensemble extends Node {
	...
	/**
	 * This method provides a means of efficiently storing the output of an Ensemble if the 
	 * component Nodes have Origins that produce SpikeOutput. 
	 *   
