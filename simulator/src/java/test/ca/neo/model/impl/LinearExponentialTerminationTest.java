...
public class LinearExponentialTerminationTest extends TestCase {
	...
	public void testReset() throws SimulationException {
		...
		let.setValues(new RealOutputImpl(new float[]{1f}, Units.ACU));
	...
	public void testSetValues() throws SimulationException {
		...
		try {
			let.setValues(new SpikeOutputImpl(new boolean[]{true}, Units.SPIKES));
		...
		let.setValues(new SpikeOutputImpl(new boolean[]{true, false, true}, Units.SPIKES));
		float current = let.updateCurrent(true, 0, 0);
		assertClose(4f, current, .01f);
		
		let.reset(false);
		
		let.setValues(new RealOutputImpl(new float[]{1f, .1f, .01f}, Units.SPIKES_PER_S));
	...
	public void testUpdateCurrent() throws SimulationException {
		...
		let.setValues(new SpikeOutputImpl(new boolean[]{false}, Units.SPIKES));
		assertClose(0, let.updateCurrent(true, 0, 0), tol);
		
		let.setValues(new RealOutputImpl(new float[]{0f}, Units.SPIKES_PER_S));
		...
		let.setValues(new RealOutputImpl(new float[]{10f}, Units.SPIKES_PER_S));
		current = 0;
		for (int i = 0; i < 120; i++) {
