...
def make_encoders(neurons, dimensions, srng, encoders=None):
    ...
    snrg: RandomStreams, theano random number generator function
    encoders: list, set of possible preferred directions of neurons
    '''
...
class Accumulator:
    def __init__(self, ensemble, t_psc):
        ...
        Parameters
        ----------
        ensemble: Ensemble class, the ensemble this set of terminations is attached to
        t_psc: float, post-synaptic time constant on filter
        '''
    ...
    def add(self, another_termination_input):
        '''
        ...
        Parameters
        ----------
        another_termination_input: theano object representing the output of the pre population multiplied by this termination's transform matrix
        '''
        if self.total is None: self.total = another_termination_input # initialize internal value storing voltage levels of neurons
        else: self.total = self.total + another_termination_input # add input current to voltage level
...
class Ensemble:
    def __init__(self, neurons, dimensions, count=1, max_rate=(200,300), intercept=(-1.0,1.0), 
        ...
        Parameters
        ----------
        neurons: int, number of neurons in this population
        dimensions: int, number of dimensions in signal these neurons represent 
        count: int, number of sub-populations - for network arrays
        max_rate: tuple, lower and upper bounds on randomly generate firing rates for neurons in this population
        intercept: tuple, lower and upper bounds on randomly generated x offset
        t_ref: float, refractory period of neurons in this population
        t_rc: float, RC constant 
        seed: int, seed value for random number generator
        neuron_type: string, type of neuron model to use, options = {'lif'}
        dt: float, time step of neurons during update step
        encoders: list, set of possible preferred directions of neurons
        '''
        self.seed = seed
    ...
    def add_origin(self, name, func):
        ...
        Parameters
        ----------
        name: string, name of origin
        func: function, desired transformation to perform over represented signal
        '''
    ...
    def add_filtered_input(self, input, t_psc):
        ...
        Parameters
        ----------
        input: theano object representing the output of the pre population multiplied by this termination's transform matrix
        t_psc: float, post-synaptic time constant
        '''
